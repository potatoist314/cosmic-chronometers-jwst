...
scale = 1e9 / 3631e-23
plot_flux = np.where(mask, flux * scale, np.nan)

fig, axes = plt.subplots(2, 1, figsize=(13, 7), sharex=True, gridspec_kw={"height_ratios": [3, 1]})
axes[0].plot(wave, plot_flux, color="0.15", lw=0.55, label="LEGA-C")
for index, prediction in enumerate(default_spectra):
    plot_prediction = np.where(mask, prediction * scale, np.nan)
    axes[0].plot(wave, plot_prediction, color="tab:blue", alpha=0.45, lw=0.8, label="default medians" if index == 0 else None)
for index, prediction in enumerate(fast_spectra):
    plot_prediction = np.where(mask, prediction * scale, np.nan)
    axes[0].plot(wave, plot_prediction, color="tab:orange", alpha=0.45, lw=0.8, label="fast-path medians" if index == 0 else None)
for index in range(4):
    difference_sigma = (fast_spectra[index] - default_spectra[index]) / uncertainty
    plot_difference_sigma = np.where(mask, difference_sigma, np.nan)
    axes[1].plot(wave, plot_difference_sigma, lw=0.65, alpha=0.7, label=str(default_runs[index][1]["seed"]))
