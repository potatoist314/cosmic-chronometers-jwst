...
<details class="model-row"><summary><span class="model-s">Dust law <span class="model-tag">inherited</span></span><span class="model-v">Kriek &amp; Conroy 2013, one diffuse screen; \(A_V = 1.086\,\tau\)</span><span class="model-w"></span><span class="model-f"></span></summary><ul><li><i>how</i> \(\tau_\lambda = \tau\,[k_{\mathrm{Calzetti}}(\lambda) + D(\lambda)]/4.05 \times (\lambda/5500\,\text{\AA})^{\delta}\)</li><li><i>how</i> bump at 2175 \(\text{\AA}\), width 350 \(\text{\AA}\), independent strength \(E_b \sim \mathrm{Uniform}(0,6)\)</li><li><i>source</i> <code>sedpy_jax.attenuation_dust.kriek_conroy_free_bump</code> through <code>DiffuseDust</code></li><li><i>how</i> <code>diffuse_bump_strength</code> in <code>PRIORS</code> selects <code>kriek_conroy_free_bump</code>: independent \(E_b\) and slope; \(E_b\) has prior Uniform(0, 6), Liu Hao, 24 Sep 2026; the original law remains available</li></ul></details>
