...
The order-10 calibration polynomial includes \(a_0\) for spectrum normalisation; no separate `spectrum_scaling` is sampled. Its constant multiplier has the unbounded prior \(1+a_0\sim\mathcal{N}(1,0.1^2)\). Photometry anchors the total flux. The spectral likelihood still samples its fractional calibration floor.
