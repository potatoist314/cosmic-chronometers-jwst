...
Free redshift
: `zred` with prior \(\operatorname{Normal}(z_{\mathrm{cat}},\sigma_z)\), \(\sigma_z=(100\,\mathrm{km\,s^{-1}}/c)(1+z_{\mathrm{cat}})\). The Spectrum keeps the projection baked at \(z_{\mathrm{cat}}\) and stretches the observed grid at each call. The stretch commutes with every smoothing kernel, so the only error is linear interpolation between 0.6 \(\text{\AA}\) pixels.
...
Code
: `Spectrum(free_z=True)` and `predict(..., zred=z)` on the fork branch `free-z-spectrum` (354f5e9), cherry-picked onto the pinned ceridwen submodule as c540bc7 on 2026-09-17, when both parameters became notebook defaults and the switches were removed. Switches `CERIDWEN_FREE_ZRED_KMS` (100) and `CERIDWEN_FREE_SIGMA_FRAC` (0.2) in `notebooks/ceridwen_integrated_photometry_spectra.ipynb`. Since 2026-09-15 the notebook switch is `CERIDWEN_FREE_SIGMA` (0/1); since 2026-09-18 both parameters are sampled when `PRIORS` in the top cell has a `zred` and a `sigma_smooth` entry, with no switch and the `sigma_smooth` prior is \(\operatorname{Normal}(\sigma_{\mathrm{cat}},\sigma_{\mathrm{cat,err}})\) clipped at \(\pm 3\,\sigma_{\mathrm{cat,err}}\), with \(\sigma_{\mathrm{cat,err}}\) the DR2 `SIGMA_STARS_PRIME_err`; the fits on this page used the earlier 0.2 \(\sigma_{\mathrm{cat}}\) prior.
...
<figcaption>\(\Delta v\) and \(\sigma_{\mathrm{smooth}}\) posteriors against the DR2 \(\sigma_{\mathrm{cat}}\) (dashed), the CPU redshift scan (dotted) and the prior bounds (shaded), with M5_173928 railed at 1.5 \(\sigma_{\mathrm{cat}}\)</figcaption>
</figure>
...
Every delta is `zsig` minus `poly3_total` of the same galaxy and seed. Raw \(\chi^2\) is the spectral \(\chi^2\) with the pixel errors alone, comparable across arms. Stored \(\chi^2\) uses each fit's own \(f_{\mathrm{calib}}\).

| galaxy | \(\mathrm{S/N}\) | \(\Delta v\) [\(\mathrm{km\,s^{-1}}\)] | CPU scan | \(\sigma_{\mathrm{smooth}}\) [\(\mathrm{km\,s^{-1}}\)] | DR2 \(\sigma_{\mathrm{cat}}\) | pull | Delta raw \(\chi^2\) | Delta stored \(\chi^2\) | Delta phot \(\chi^2\) | \(\Delta\ln Z\) |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| M5_173928 | 13 | +7.8 \(\pm\) 9.4 | 0 | 267.8 \(\pm\) 1.8 | 179.7 \(\pm\) 13.3 | +6.6, at 1.5 \(\sigma_{\mathrm{cat}}\) | -535 | -3.1 | -11.8 | +73.3 |
| M12_185653 | 22 | +12.3 \(\pm\) 6.0 | 0 | 144.2 \(\pm\) 5.4 | 173.8 \(\pm\) 10.2 | -2.9 | -66 | -19.6 | -0.4 | +7.4 |
| M1_206545 | 31 | -19.6 \(\pm\) 4.4 | -21 | 241.3 \(\pm\) 4.9 | 210.0 \(\pm\) 8.0 | +3.9 | -71 | -32.8 | -10.5 | +32.7 |
| M5_172669 | 105 | +18.3 \(\pm\) 3.2 | +52 | 231.5 \(\pm\) 3.9 | 223.8 \(\pm\) 6.1 | +1.3 | -66 | +39.4 | +1.1 | +4.4 |

| galaxy | Delta \(\log M\) | Delta \(t_{50}\) [Gyr] | Delta age [Gyr] | Delta \(\tau_{\mathrm{dust}}\) | Delta \(f_{\mathrm{calib}}\) | wall zsig [s] | wall poly3_total [s] |
| --- | --- | --- | --- | --- | --- | --- | --- |
