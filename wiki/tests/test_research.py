...
class ResearchTests(unittest.TestCase):
    ...
    def test_roadmap_preserves_scores_dependencies_and_original_amendments(self):
        ...
        rows = re.findall(r'<tr id="[^"]+"[^>]*>(.*?)</tr>', page, re.S)
        self.assertEqual(len(rows), 4)
        self.assertTrue(all('<input type="checkbox">' in row for row in rows))
        self.assertNotIn('<input type="checkbox" checked>', page)
        self.assertEqual(page.count('<p class="roadmap-status" role="status"></p><table class="roadmap">'), 2)
